...
void bfson2d(vv32 &arr,ll val,vv64 &vis)
{
    ll cc=0;
    while(q.size()&&q.begin()->first<val)
    {
        auto &a=*q.begin();
        int currx = a.se.first;
        int curry = a.se.second;
        q.erase(q.begin());
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            if (isvalid(currx + dx[i], curry + dy[i]))
            {
                if(vis[x1][y1]==0)
                {
                    q.ie({arr[x1][y1],{x1,y1}});
                }
            }
        }
    }
    // cout<<"CURRENT NODES COUNT"<<endl;
    // cout<<cc<<endl;
    ans.pb(cc);
}
