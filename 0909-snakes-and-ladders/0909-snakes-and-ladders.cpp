...
ll n;
void bfs(uv64 &adj, vv64 &dist, ll src)
{
    queue<p64> q;
    q.push({src,0});
    dist[src][0] = 0;
    while (!q.empty())
    {
        ...
        if(type==0 && adj[curr].size()>0)
        {
            for(auto child:adj[curr])
            {
                if (dist[child][1]>dist[curr][0])
                {
                    dist[child][1] = dist[curr][0];
                    q.push({child,1});
                }
            }
        }
        else
        {
            forsn(i,1,7)
            {
                ...
                if(child <= n*n)
                {
                    if(dist[child][0]>dist[curr][type]+1)
                    {
                        dist[child][0]=dist[curr][type]+1;
                        q.push({child,0});
                    }
                }
            }
